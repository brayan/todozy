...
internal class TaskRepositoryImpl(
    ...
    override suspend fun getBeforeTodayTasks(filter: TaskFilter): Result<List<Task>> =
        runCatching {
    ...
    override suspend fun getTodayTasks(filter: TaskFilter): Result<List<Task>> =
        runCatching {
            val tasksData = taskLocalDataSource.getTodayTasks(filter).getOrThrow()
            return@runCatching tasksData.loadAlarmsAndMapToTasks()
        }

    override suspend fun getTomorrowTasks(filter: TaskFilter): Result<List<Task>> =
        runCatching {
            val tasksData = taskLocalDataSource.getTomorrowTasks(filter).getOrThrow()
            return@runCatching tasksData.loadAlarmsAndMapToTasks()
        }

    override suspend fun getNextDaysTasks(filter: TaskFilter): Result<List<Task>> =
        runCatching {
            val tasksData = taskLocalDataSource.getNextDaysTasks(filter).getOrThrow()
            return@runCatching tasksData.loadAlarmsAndMapToTasks()
        }

    override suspend fun getBeforeNowTasks(): Result<List<Task>> =
        runCatching {
            val tasksData = taskLocalDataSource.getTasksThrowBeforeNow().getOrThrow()
            return@runCatching tasksData.loadAlarmsAndMapToTasks()
        }

    override suspend fun getTasksWithAlarms(): Result<List<Task>> =
        runCatching {
    ...
    override suspend fun disableTask(task: Task): Result<Task> =
        runCatching {
            taskLocalDataSource.update(task.mapToTaskData(), false)
            return@runCatching task
        }

    private suspend fun List<TaskData>.loadAlarmsAndMapToTasks() =
        map { taskData ->
            val alarm = alarmRepository.getAlarmByTaskId(taskData.id).getOrNull()
