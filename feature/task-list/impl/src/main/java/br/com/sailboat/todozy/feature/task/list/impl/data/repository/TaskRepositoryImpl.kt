...
internal class TaskRepositoryImpl(
    ...
    private suspend fun List<TaskData>.loadAlarmsAndMapToTasks(): List<Task> {
        if (isEmpty()) {
            return emptyList()
        }

        val taskIds = mapNotNull { taskData -> taskData.id.takeIf { it != Entity.NO_ID } }
        val alarmsByTaskId = alarmRepository.getAlarmsByTaskIds(taskIds).getOrElse { emptyMap() }

        return map { taskData ->
            val alarm = alarmsByTaskId[taskData.id]
